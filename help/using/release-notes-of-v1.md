...
* macOS X 10.10 ou version ultérieure, avec les derniers correctifs.

* Windows 10 avec les derniers Service Packs et correctifs.

Adobe recommande d’utiliser la dernière version de l’application de bureau AEM pour vous assurer que vous utilisez les dernières fonctionnalités, les correctifs les plus récents et les meilleures performances possibles.

La version de l’application de bureau AEM que vous prévoyez d’installer sur votre ordinateur local nécessite un serveur AEM spécifique
version/composants côté serveur supplémentaires (packs de services, correctifs logiciels ou packs de fonctionnalités). Assurez-vous que le serveur AEM est correctement configuré avant de vous y connecter pour la première fois. Si vous avez besoin d’aide, contactez votre administrateur ou administratrice AEM.
...
>[!NOTE]
>
>Si vous utilisez macOS 10.15.4 ou une version ultérieure, utilisez au moins la version 1.10.0.6 de l’application. Cette version de correctif est conforme aux [exigences de certification d’Apple](https://developer.apple.com/news/?id=04102019a).
...
**Fenêtre simplifiée de statut des ressources**. la fenêtre de statut a été simplifiée. L’onglet [!UICONTROL Uploads] affiche désormais à la fois les ressources individuelles et les chargements par dossier ou en bloc. L’onglet Chargements en bloc précédent a été supprimé.
...
**Notifications de conflits de mise à jour**. Lorsque l’application détecte un conflit lors de la mise à jour d’une ressource, elle affiche une notification permettant à l’utilisateur de le vérifier sans surveiller la fenêtre d’état. Au démarrage, l’application recherche tous les conflits et permet à l’utilisateur ou à l’utilisatrice de les résoudre.

**Amélioration de la gestion des pertes de connexion**. Les chargements en bloc sont suspendus en cas de perte de connexion et l’utilisateur peut reprendre ultérieurement. Une option [!UICONTROL Retry] est disponible pour effectuer une nouvelle tentative en cas d’échec du téléchargement d’un fichier.
...
* **Version 1.9/1.9.1** : chargements pouvant être repris, fenêtre de statut améliorée, icônes d’application indiquant le statut de l’application/de la connexion, prérécupération des ressources liées pour les fichiers InDesign.

* **Version 1.8** : meilleur contrôle de la taille du cache pour l’utilisateur, amélioration de l’expérience de connexion pour SAML/SSO sous Windows, prise en charge `.pac` proxy réseau sous Mac et problèmes signalés par le client.
...
La version 1.9.1 de l’appli de bureau [!DNL Adobe Experience Manager] était une version de correctif. Il a été conçu pour résoudre les principaux problèmes des clients lors du passage en caisse des ressources. De plus, vous pouvez copier des fichiers d’un partage réseau vers un répertoire local.
...
* Prise en charge d’une copie d’un dossier mappé vers un dossier local lorsque le dossier d’utilisateurs se trouve sur une partition de disque distincte (CQ-4243978)
...
**Chargements pouvant être repris**
Pour les chargements, en particulier autour des fichiers volumineux, il existe une option pour les suspendre/reprendre dans la nouvelle fenêtre Statut de la ressource .

**Fenêtre de statut de ressource améliorée**
Une fenêtre Statut de ressource améliorée fournit les informations suivantes sur les ressources.
...
* Les fichiers en conflit s’affichent avec une icône sur laquelle l’utilisateur ou l’utilisatrice peut cliquer. Cliquez sur l’icône pour afficher une boîte de dialogue contenant une explication et deux options :
...
* Les téléchargements terminés s’affichent avec le total transféré, le taux final et une icône qui ouvre le fichier lorsque l’utilisateur clique dessus (disponible uniquement pour des fichiers uniques).
...
* Le pied de page indique le nombre total de fichiers téléchargés et le taux de transfert moyen.

* Si un utilisateur choisit d’ouvrir ou de modifier plusieurs fichiers à partir de l’interface Web d’[!DNL Experience Manager Assets], ils sont regroupés. Par exemple, myasset.jpeg et 4 autres fichiers.

* Lors du téléchargement de documents InDesign avec des ressources liées à partir d’AEM Assets, l’application de bureau télécharge d’abord toutes les ressources liées avant d’ouvrir le document et d’indiquer le statut du téléchargement. Par exemple, 5 sur 24.

[!UICONTROL Bulk Uploads]

Le téléchargement de hiérarchies de dossiers volumineuses via [!UICONTROL Create] > [!UICONTROL Upload Folder] dans l’interface utilisateur web d’AEM déclenche cette boîte de dialogue. Cela se produit également lors de la copie et de la sélection de « Coller Assets » dans le Finder ou l’Explorateur dans le menu contextuel de l’appli de bureau.

* Affiche les chargements en cours, y compris une barre de progression et le nom du fichier en cours de transfert.

* Les chargements en cours comprennent une icône qui annule le chargement lorsque l’utilisateur ou l’utilisatrice clique dessus. Le transfert s’arrête une fois le transfert du fichier en cours terminé.
...
* Si le transfert d’un fichier individuel échoue, il s’affiche dans l’onglet en tant qu’erreur. Dans le cas contraire, les fichiers individuels n’affichent pas dans l’onglet * une seule entrée pour l’ensemble du chargement.

**Icônes indiquant le statut des opérations en arrière-plan**

L’icône de l’application indique l’état des opérations en arrière-plan afin de fournir un meilleur indice visuel aux utilisateurs et utilisatrices. Par exemple, lorsque l’application n’est pas connectée à AEM, l’icône est grisée. Lorsqu’il y a un chargement actif, il affiche une superposition « sync », etc.

**Prérécupération des ressources liées**

Pour améliorer l’expérience utilisateur avec les documents InDesign contenant des ressources liées stockées dans AEM, l’application de bureau prérécupère ces fichiers liés dans le cache local. Ce flux se produit avant le téléchargement et l’ouverture du document InDesign. Ainsi, l’utilisateur dispose des fichiers liés localement et n’a pas besoin d’attendre plus longtemps lors de l’accès aux ressources dans InDesign (dans le panneau Liens).
La pré-récupération ne fonctionne que si AEM reconnaît les liens côté serveur. Une ressource avec des liens reconnus comporte une liste de « Références » répertoriée dans la vue Propriétés de la ressource InDesign.
...
* Connexion : (Windows) connexion fixe à l’instance AEM configurée pour utiliser SAML et SSL (CQ-4216353)

* Réseau :

   * Lorsqu’une session AEM expire, l’utilisateur est maintenant averti et peut cliquer sur la notification pour se reconnecter (CQ-4202028).

   * (Mac) Ajoutez la prise en charge de la connexion à AEM au moyen de la configuration de proxy `.pac` (CQ-4233430).

   * (Windows) Résolution de problèmes relatifs à la boîte de dialogue Advanced > Login URL (Avancé > URL de connexion) (CQ-4236061).

* Correctifs :

   * Boîte de dialogue Informations sur les ressources supplémentaires : la barre d’actions n’était parfois pas visible (CQ-4208540).
...
* Stabilité :
   ...
   * Amélioration de la stabilité lorsque l’application de bureau AEM se connecte à un serveur AEM surchargé (CQ-4224803).
...
* Connexion : permet de se connecter avec strictSSL désactivé pour les configurations spéciales (comme les certificats émis localement) (CQ-4223949).
...
* Installation et désinstallation :
   ...
   * [Windows 32 bits] le programme d&#39;installation ne parvient pas à installer Microsoft .NET Framework v. 4.5 (CQ-4218084).

   * (Mac) Script manuel pour la suppression complète des fichiers de l’appli de bureau (CQ-4216489).

>[!NOTE]
>
>Les problèmes détectés dans les chargements bêta de l’application de bureau AEM 1.7 qui étaient absents de la version 1.6 sont omis des notes de mise à jour.
...
* Amélioration du processus de connexion à AEM :
   ...
   * Ajoutez la possibilité de configurer une URL de connexion distincte dans les Préférences (CQ-4214052, CQ-4214051).

* Convivialité : avertissez l’utilisateur lorsqu’une ressource est toujours en cours de téléchargement pour des ressources plus volumineuses (CQ-4216284).
...
* Mise en cache et accès au partage de réseau dans le Finder/l’Explorateur :
   ...
   * L’icône de verrou est désormais visible pour les ressources extraites sous Windows 10 (CQ-90957).
...
* Correctifs en matière de stabilité et de performances :
   ...
   * L’appli de bureau 1.5 peut ralentir un ordinateur de bureau au fil du temps (CQ-4209159).
...
* Nouvelles fonctionnalités et amélioration : ajout de la fonctionnalité Copier/Coller à l’intégration du Finder pour permettre un transfert direct de Desktop vers AEM (CQ-4208158).
...
* Correctifs :
   ...
   * Le point de montage de l’appli de bureau est désormais /Volumes/DAM afin d’être cohérent pour tous les utilisateurs (CQ-4208159).

   * Le premier placement d’un fichier dans InDesign déclenche un avertissement de mise à jour (CQ-4207454).

Remarque concernant les avertissements relatifs aux liens : les applications Creative Cloud (par exemple, InDesign) prennent un « instantané » de la date et de l’heure de la dernière modification de l’élément au moment où il est placé. Si cette date est modifiée ultérieurement, l’application Adobe Creative Cloud signale que les liens sont obsolètes. Ces informations sont présentées de plusieurs façons :

* Lorsque l’application Adobe Creative Cloud est lancée, elle affiche une boîte de dialogue informant l’utilisateur ou l’utilisatrice que les ressources liées sont obsolètes, et l’invitant à prendre des mesures.
...
Ce comportement est identique pour les ressources sur le disque local et pour les ressources d’un répertoire monté sur le bureau AEM, avec les exceptions suivantes :

* Si un autre utilisateur modifie une ressource placée, l’icône d’avertissement s’affiche la première fois que d’autres utilisateurs ouvrent un document contenant la ressource placée. Cet avertissement ne se produit que si la ressource placée a déjà été mise en cache localement.

* Si un utilisateur modifie une ressource placée dans le répertoire monté du bureau AEM, puis efface son cache local, la ressource placée est signalée comme étant obsolète.

Ces deux cas sont attendus et constituent des effets secondaires de l’architecture de « synchronisation retardée » d’AEM Desktop.

**Version 1.5.0.x pour macOS X et Windows :** Cette version de l’application de bureau AEM offre les avantages suivants :

* Amélioration de la stabilité et de la résilience face aux problèmes de mise en réseau.
...
* Amélioration de la fiabilité de l’ouverture, du placement et de l’enregistrement des ressources d’AEM dans les applications Creative Cloud (CQ-4203968, CQ-4205511, CQ-103543, CQ-4207141, CQ-90980).
...
* Autres correctifs :
   * Meilleure prise en charge des ressources et des dossiers dont le nom contient des caractères japonais ou dont les paramètres de langue ne sont pas en anglais (CQ-4195433, CQ-4205793, CQ-4199446).

   * Gestion des connexions améliorée avec SSL (CQ-4200217).
...
* Simplification du chargement des dossiers hiérarchiques par l’intermédiaire de la nouvelle opération Create > Upload Folder (Créer > Charger un dossier) de l’interface utilisateur tactile.
   * Cette action lance une opération de chargement de dossier effectuée par l’application de bureau .
   * L’application de bureau parcourt la hiérarchie de dossiers donnée sur le bureau en arrière-plan et charge les fichiers dans AEM Assets.
   * L’utilisateur peut surveiller la progression des opérations en cours dans la nouvelle fenêtre Statut de la file d’attente de chargement.
   * Le Statut de la file d’attente de chargement fournit également de meilleures informations de dépannage (par exemple, aucune connexion au serveur).
...
* Correctifs signalés par le client ou la cliente.
...
* Amélioration de l’intégration avec macOS (le protocole de lecteur réseau local est passé de WebDAV à SMB1 plus stable).
* L’application de bureau se connecte au serveur AEM à l’aide du protocole RESTful HTTP natif d’AEM.
...
* Une boîte de dialogue est fournie pour effectuer le suivi du statut des tâches de chargement en arrière-plan.

## Liste des modifications {#list-of-changes}

### Point de montage sous Mac {#mount-point-on-mac}

Depuis macOS 10.12 (Sierra), Apple a modifié les autorisations sur le dossier /Volumes utilisé pour monter les lecteurs et appareils réseau afin d’être plus restrictif. La création d’un nouveau point de montage à cet emplacement nécessitait des droits d’administration. Ce problème a été corrigé dans macOS 10.12.5.

Le point de montage de l’appli de bureau AEM a été modifié dans les versions 1.4 et 1.5. Sous macOS, il a été transformé en sous-dossier de gestion des ressources numériques dans le dossier local de l’utilisateur, prenant en charge les utilisateurs non administrateurs (CQ-104183).

Étant donné que le dossier `/Volumes` ne nécessite plus de droits d’administration, cette modification a été annulée dans la version 1.5.1. Cette modification permet également de partager des documents InDesign qui ont placé des ressources d’AEM entre les utilisateurs de macOS.
...
* MACOS X :
   ...
   * Le référentiel AEM monté avec l’application de bureau est visible sous la forme d’un lecteur réseau `smb` dans le Finder, au lieu d’un lecteur WebDAV.
* Windows :
   * Le protocole de lecteur réseau local pour les intégrations de bureau Windows reste inchangé ; AEM est monté en tant que partage WebDAV.
* Pour les deux plateformes (Windows et Mac) :
   * Le protocole d’accès/de téléchargement des ressources et de chargement des modifications dans AEM a été remplacé par le protocole natif AEM, qui est un protocole RESTful basé sur HTTP. Il offre un meilleur contrôle sur les opérations réseau et est davantage compatible avec l’infrastructure réseau.

>[!NOTE]
>
>Sous macOS X, le changement de protocole de lecteur réseau local de WebDAV à SMB1 entraîne un chemin d’accès local différent vers la même ressource dans le référentiel. Cette modification peut avoir un impact sur les liens vers les fichiers placés dans les applications Adobe Creative Cloud par le biais de la commande « Placer ». Voir [Utilisation de l’appli de bureau AEM](use-app-v1.md) pour plus d’informations.
...
* Les fichiers sont enregistrés à un emplacement de lecteur réseau monté par le biais de l’application de bureau en deux phases.
...
* Lors de la seconde phase, l’application de bureau charge un fichier mis à jour sur le serveur AEM après un délai prédéfini (trente secondes, par exemple). Cette opération s’effectue en arrière-plan. Utilisez l’option **Show Background File Sync Status** (Afficher le statut de synchronisation du fichier en arrière-plan) pour afficher le statut de l’opération de chargement.

## Remarques importantes {#important-notices}

**Chargement de dossier.** Adobe vous recommande d’utiliser la nouvelle fonctionnalité de chargement de dossiers pour charger des dossiers hiérarchiques plus volumineux dans AEM. Cette approche est recommandée par opposition à l’utilisation d’une opération de copie/glisser-déposer dans un référentiel AEM monté à partir du Finder/de l’Explorateur. Lors de l’utilisation de la fonction de chargement de dossiers, l’application de bureau communique directement avec AEM et dispose ainsi d’un bien meilleur contrôle sur l’ensemble du processus.

**Gardez la session AEM disponible.** L’application de bureau AEM dépend d’une session ouverte sur le serveur AEM Assets pour en assurer le bon fonctionnement. Les utilisateurs quotidiens doivent démonter AEM Assets à la fin de la journée pour se déconnecter, puis remonter le matin pour garantir les fonctionnalités de connexion et de partage réseau.

**Désactiver l’aperçu des icônes dans le Finder.** Pour une navigation performante dans les dossiers volumineux avec le Finder, en particulier avec une mauvaise connectivité réseau, assurez-vous que les options Icône et Aperçu de l’icône sont désactivées. Sinon, le Finder commence à télécharger chaque ressource dans un dossier pour générer un aperçu de petite taille, ce qui peut entraîner de faibles performances et une utilisation élevée de la bande passante (CQ-4219779)
...
**Nettoyez le cache lors de la connexion à un nouveau serveur AEM.** Si l’application de bureau se connecte à un autre serveur AEM avec la même URL, le cache n’est pas effacé automatiquement. Effacez manuellement le cache pour garantir des opérations correctes. Notez que ce processus se produit généralement lors du test, lorsque les installations AEM peuvent être remplacées lors de l’exécution sur la même URL (CQ-4216982)

**Utilisez des certificats SSL signés par une autorité de certification.** L’application de bureau AEM ne prend pas en charge les certificats SSL auto-signés lors de la connexion à AEM au moyen d’une connexion sécurisée HTTPS. Un certificat signé par une autorité de certification est requis sur le serveur pour ce type de connexion (CQ-87941).
...
* Noms de fichiers/localisation :
   * Les noms de fichiers et de dossiers comportant des caractères réservés ne sont pas gérés correctement. Veillez à utiliser des noms de fichier et de dossier adaptés aux exigences d’AEM. (CQ-93361, CQ-93308, CQ-89276, CQ-4217183)
   * Certaines applications telles que Adobe Illustrator peuvent créer des fichiers dont les noms ne sont pas pris en charge dans AEM. Par exemple, l’ajout de `Converted` après la conversion d’un fichier, ce qui empêche son chargement. (CQ-4216985)
...
* Archiver et Extraire :
   ...
   * Certaines applications ne prennent pas correctement en charge l’indicateur en lecture seule, qui permet d’enregistrer un fichier extrait par un autre utilisateur. Le fichier modifié n’est pas transféré tant que l’autre personne n’a pas archivé le fichier. Les deux modifications sont disponibles dans AEM en tant que versions différentes de la ressource. (CQ-89551, CQ-87572, CQ-89615)
   * Les statuts extrait et lecture seule sont signalés indépendamment dans le Finder. Cette approche génère deux icônes de verrouillage lorsqu’un utilisateur extrait une ressource. (CQ-89507)
* Intégration du Finder :
   * Lorsque vous faites glisser et déposez des fichiers volumineux, le Finder peut expirer pendant le transfert des fichiers en arrière-plan. Ce délai entraîne une `Error - 36`. La solution consiste à faire glisser et à déposer la ressource ou à l’ouvrir à nouveau. (CQ-4219628)
   * Le rechargement manuel de dossier ne fonctionne pas toujours. Solution : attendez trente secondes que le dossier soit automatiquement mis à jour. (CQ-97389).
   * Plus d’informations sur les ressources... est limité aux sélections de fichiers uniques. (CQ-89542, CQ-87656)
   * L’option Ouvrir dans AEM Assets... se limite aux sélections de fichiers et de dossiers uniques. (CQ-83382)
...
* Windows :
   * Les fichiers avec des flux de données alternatifs (ADS, Alternate Data Streams) ne sont entièrement pris en charge que par NTFS. Lors de la copie de fichiers dans le partage WebDAV par le biais de l’application de bureau, une boîte de dialogue d’avertissement vous avertit que certaines propriétés de fichier ne peuvent pas être transférées vers le nouvel emplacement. Cet avertissement est généralement correct, car les propriétés ne sont pertinentes que pour une application particulière sur le bureau de l’utilisateur et n’ont rien à voir avec le contenu réel du fichier. (CQ-103770) (Windows)
   * L’utilisateur ou l’utilisatrice qui utilise l’application de bureau sous Windows doit être celui ou celle qui l’installe. (CQ-4216389) (Windows)
   * L’application peut se bloquer lors de la sélection de l’option [!UICONTROL Retry] en cas d’échec du chargement. Ce blocage peut se produire dans certains cas après la reprise du chargement par lots après déconnexion. (CQ-4251884) (Windows)
