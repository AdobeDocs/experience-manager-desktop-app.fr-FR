...
Cette intégration permet une gestion et un accès centralisés aux ressources dans Creative Cloud et d’autres applications, garantissant ainsi la conformité avec les normes de marque et autres.

L’appli de bureau [!DNL Experience Manager] v1 vous permet d’effectuer les tâches clés suivantes :

1. [Connexion avec un serveur [!DNL Experience Manager] &#x200B;](#installandconnect)
1. [Ouvrir des ressources directement sur l’application de bureau](#openondesktop)
1. [Modification et extraction de ressources à partir de l’application de bureau](#workonassets)
1. [Chargement en masse de ressources et de dossiers](#bulkupload)

Pour les pratiques recommandées, voir les [bonnes pratiques d’utilisation de l’appli de bureau](best-practices-for-v1.md). Si vous rencontrez des problèmes lors de l’utilisation de l’application, consultez Comment [résoudre les problèmes liés  [!DNL Experience Manager]  l’application de bureau](troubleshoot-app-v1.md).
...
## Installation et connexion de l’application au serveur [!DNL Experience Manager] {#installandconnect}
...
1. [Activer les actions de bureau](#desktopactions) sur le serveur [!DNL Experience Manager].

L’application de bureau [!DNL Experience Manager] utilise une connexion HTTPS pour se connecter au serveur [!DNL Experience Manager] afin de transférer vos ressources de manière robuste et sécurisée.

>[!NOTE]
>
>Pour tout ou partie des étapes d’installation et de configuration, vous aurez peut-être besoin de l’aide de votre administrateur [!DNL Experience Manager] ou de votre administrateur système.

### Installation de l’application {#installapp}

Assurez-vous que l’application prend en charge votre version du serveur Experience Manager pour utiliser l’application de bureau Experience Manager. Téléchargez le fichier d’installation approprié (binaire) pour votre système d’exploitation (Mac ou Windows) et installez ensuite l’application.
...
L’application autorise les paramètres de connexion et de déconnexion des serveurs [!DNL Experience Manager], d’affichage de l’état des chargements, de gestion du cache local, etc. Les paramètres par défaut sont opérationnels pour un utilisateur ordinaire de l’application. Vous pouvez ajuster les paramètres pour tirer le meilleur parti de l’application. Tirez également le meilleur parti de l’intégration au serveur [!DNL Experience Manager]. Les différents paramètres sont les suivants :
...
**Options** Cliquez sur options dans la barre d&#39;état de l&#39;application de bureau pour définir l&#39;application à lancer au démarrage, se connecter au serveur [!DNL Experience Manager] au lancement et modifier la lettre de lecteur local pour [!DNL Assets] après le montage.

**Advanced > Manage cache (Avancé > Gérer le cache)** Vous pouvez contrôler la quantité d’espace disque disponible pour la mise en cache locale. Les artefacts du serveur [!DNL Assets] sont mis en cache localement pour garantir une expérience plus fluide. Vous pouvez modifier les valeurs par défaut en fonction de vos besoins. Vous pouvez également vider le cache pour récupérer à nouveau toutes les ressources. Lorsque vous effacez le cache, les modifications non enregistrées sont conservées. Toutes les ressources non archivées dans le serveur [!DNL Experience Manager] sont conservées et non supprimées.
...
1. Lancez l’application de bureau [!DNL Experience Manager]. Pour mapper votre instance avec l’application [!DNL Experience Manager], spécifiez votre serveur [!DNL Experience Manager] au format `https://[aem-server-url]:[port]`.
...
Lorsque vous cliquez sur **Ouvrir** pour ouvrir une ressource sur votre ordinateur local, l’application télécharge la ressource dans son cache interne. L’application lance l’appli de bureau native associée au type de fichier de la ressource téléchargée.

Sur Mac, sélectionnez **Ouvrir** dans le menu contextuel pour ouvrir une ressource via l’application de bureau [!DNL Experience Manager]. Sous Windows, sélectionnez Ouvrir sur le web dans le menu contextuel pour ouvrir la ressource. Dans la fenêtre Asset Status (Statut de la ressource), cliquez/appuyez sur ![Icône Open on Desktop](assets/do-not-localize/aemassets_icon_openondesktop.png) pour ouvrir la ressource.
...
*Image : options de menu contextuel permettant d’accéder et d’ouvrir des ressources à l’aide de l’application de bureau [!DNL Experience Manager].*
...
1. Ouvrez l’interface d’[!DNL Assets] et placez le pointeur sur un dossier ou une ressource afin d’afficher les actions de bureau sous la forme d’actions rapides dans le mode Carte.
   ...
   *Image : dans l’interface utilisateur d’Assets, ouvrez le menu des actions rapides pour afficher les actions de bureau.*
...
Pour extraire des ressources à partir de l’interface web d’[!DNL Experience Manager], deux méthodes sont possibles. Pour plus d’informations sur la première méthode, reportez-vous à la section [Archivage et extraction de fichiers à partir de l’interface utilisateur d’Assets](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/assets/managing/check-out-and-submit-assets). Pour les deuxièmes méthodes d’extraction et d’ouverture de la ressource lors de l’installation de l’application de bureau [!DNL Experience Manager], procédez comme suit.
...
1. Pour ouvrir la ressource, cliquez/appuyez sur l’action rapide Open on desktop (Ouvrir sur le Bureau) ![Icône Open on Desktop](assets/do-not-localize/aemassets_icon_openondesktop.png).
   ...
   >[!NOTE]
   >
   >Lorsque vous modifiez un fichier ouvert mais non extrait, les autres utilisateurs ne savent pas que vous mettez à jour la ressource.

1. Pour ouvrir une ressource à modifier dans une application Adobe Creative Cloud, cliquez sur ![icône Modifier le bureau](assets/do-not-localize/aemassets_icon_editdesktop.png). Cette option extrait également la ressource pour modification. Après avoir procédé aux modifications, archivez la ressource pour mettre à jour les modifications dans [!DNL Assets].
...
### Extraction de ressources à partir de l’outil de recherche sur macOS {#check-out-assets-on-mac}

L’application vous permet d’extraire des fichiers de ressource pour empêcher d’autres utilisateurs de modifier les fichiers sur lesquels vous travaillez.
...
1. Dans la boîte de dialogue Informations sur les ressources, cliquez sur l’icône Extraire . L’icône Extraire passe en icône d’archivage.
...
1. Consultez la ressource dans l’Explorateur. L’icône de verrouillage sur la ressource ![Icône Asset lock](assets/do-not-localize/aemassets_icon_lockcheckout.png) indique que vous avez extrait cette dernière.
   ...
   >[!NOTE]
   >
   >L’icône de verrouillage peut s’afficher après un certain délai. L’application de bureau [!DNL Experience Manager] met en cache les ressources pour un accès rapide. La mise à jour du statut verrouillé peut donc prendre quelques instants.
...
À l’aide de l’application de bureau [!DNL Experience Manager], vous pouvez charger un dossier entier contenant des ressources de votre répertoire de fichiers local vers [!DNL Assets]. Ainsi, toutes les ressources du dossier sont chargées en bloc et non une par une.

1. Dans l’interface utilisateur d’Assets, cliquez/appuyez sur **Créer** dans la barre d’outils, puis, dans le menu, sélectionnez **Charger le dossier**.
...
1. Cliquez/appuyez sur OK. La boîte de dialogue Assets Status (Statut des ressources) affiche le statut du chargement.
   ...
   Consultez le statut du chargement dans la fenêtre Statut de la ressource .
...
Adobe déconseille de copier-coller ou de faire glisser un plus grand nombre de fichiers ou de dossiers imbriqués du système de fichiers local vers la zone de partage réseau. L’application ne peut pas contrôler le processus de chargement en raison de limitations techniques et de faibles performances.

Vous pouvez également sélectionner des fichiers/dossiers dans le Finder ou l’Explorateur, les copier, accéder au dossier cible dans la zone de partage réseau et choisir **Coller Assets** dans le menu contextuel de l’application de bureau [!DNL Experience Manager]. Ainsi, l’application de bureau [!DNL Experience Manager] commence à charger les ressources collées à l’instar de l’option **Charger le dossier** disponible dans l’interface web [!DNL Experience Manager].
